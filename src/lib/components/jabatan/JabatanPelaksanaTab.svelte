<script>
	...
	async function loadData() {
		...
		error = null;
		try {
			const res = await api(`/ref-jabatan/pelaksana?search=${search}&page=${page}`);
			data = res.data;
			meta = res.meta;
		} catch (err) {
			error = err.message;
	...
	function openCreate() {
		isEditing = false;
		formData = { id: '', urusan_id: '', suburusan_id: '', nmJab: '' };
		fieldErrors = {};
		formError = null;
		showModal = true;
	}
	...
	function openEdit(item) {
		...
		formData = { 
			...
			suburusan_id: item.suburusan_id, 
			nmJab: item.nmJab 
		};
		fieldErrors = {};
		formError = null;
	...
	async function handleSubmit() {
		...
		try {
			const payload = {
				urusan_id: parseInt(formData.urusan_id),
				suburusan_id: parseInt(formData.suburusan_id),
				nmJab: formData.nmJab
			};

			if (isEditing) {
				await api(`/ref-jabatan/pelaksana/${formData.id}`, {
					method: 'PATCH',
					body: JSON.stringify(payload)
				});
				toast.success('Jabatan pelaksana berhasil diperbarui');
			} else {
				await api('/ref-jabatan/pelaksana', {
					method: 'POST',
					body: JSON.stringify(payload)
				});
				toast.success('Jabatan pelaksana berhasil ditambahkan');
			}
			showModal = false;
			loadData();
		} catch (err) {
	...
	async function executeDelete() {
		deleteLoading = true;
		try {
			await api(`/ref-jabatan/pelaksana/${itemToDelete.id}`, { method: 'DELETE' });
			toast.success('Jabatan pelaksana berhasil dihapus');
	...
	<div class="flex justify-between items-center">
		<div class="relative w-72">
			...
			<input
				type="text"
				bind:value={search}
				placeholder="Cari jabatan pelaksana..."
			...
			<table class="w-full text-left text-sm">
				<thead class="bg-zinc-50/50 dark:bg-zinc-800/50 text-zinc-500 dark:text-zinc-400 font-bold uppercase tracking-wider text-[10px] border-b border-zinc-200 dark:border-zinc-800">
					<tr>
						...
						<th class="px-6 py-4 text-center">Urusan</th>
						<th class="px-6 py-4 text-center">Sub Urusan</th>
					...
					{#each data as item}
						<tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
							...
							<td class="px-6 py-4 text-center">
								<span class="px-2 py-1 bg-zinc-100 dark:bg-zinc-800 rounded font-mono text-[10px]">
									{item.urusan_id}
								</span>
							</td>
							<td class="px-6 py-4 text-center">
		...
		<div class="bg-white dark:bg-zinc-900 w-full max-w-md rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
			<div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-900/50">
				<h2 class="text-sm font-black uppercase tracking-widest text-zinc-900 dark:text-zinc-50">
					{isEditing ? 'Ubah Jabatan Pelaksana' : 'Tambah Jabatan Pelaksana'}
				</h2>
			...
			<div class="p-6 space-y-5">
				...
				<Input
					...
					placeholder="Masukkan nama jabatan pelaksana..."
				...
				<div class="grid grid-cols-2 gap-4">
					<Input
						label="ID Urusan"
						type="number"
						bind:value={formData.urusan_id}
						placeholder="ID"
						error={fieldErrors.urusan_id}
						required
					/>
					<Input
						label="ID Sub Urusan"
						type="number"
						bind:value={formData.suburusan_id}
						placeholder="ID"
						error={fieldErrors.suburusan_id}
						required
					/>
				</div>
			</div>
