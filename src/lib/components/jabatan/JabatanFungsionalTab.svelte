<script>
	...
	async function loadData() {
		...
		error = null;
		try {
			const res = await api(`/ref-jabatan/fungsional?search=${search}&page=${page}`);
			data = res.data;
			meta = res.meta;
		} catch (err) {
			error = err.message;
	...
	function openCreate() {
		isEditing = false;
		formData = { id: '', nmJab: '', jenjangJab_id: '', ket: '', kelompok: 'Teknis' };
		fieldErrors = {};
		formError = null;
		showModal = true;
	}
	...
	function openEdit(item) {
		...
		formData = { 
			id: item.id, 
			nmJab: item.nmJab, 
			jenjangJab_id: item.jenjangJab_id || '', 
			ket: item.ket || '', 
			kelompok: item.kelompok || 'Teknis'
		};
		fieldErrors = {};
		formError = null;
		showModal = true;
	...
	async function handleSubmit() {
		...
		try {
			if (isEditing) {
				await api(`/ref-jabatan/fungsional/${formData.id}`, {
					method: 'PATCH',
					body: JSON.stringify(formData)
				});
				toast.success('Jabatan fungsional berhasil diperbarui');
			} else {
				await api('/ref-jabatan/fungsional', {
					method: 'POST',
					body: JSON.stringify(formData)
				});
	...
	async function executeDelete() {
		deleteLoading = true;
		try {
			await api(`/ref-jabatan/fungsional/${itemToDelete.id}`, { method: 'DELETE' });
					...
					{#each data as item}
						<tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
							...
							<td class="px-6 py-4 whitespace-nowrap">
								<span class="px-2 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-[10px] font-bold">
									{item.kelompok}
								</span>
							</td>
							<td class="px-6 py-4 text-zinc-500 dark:text-zinc-400 max-w-xs truncate">{item.ket || '-'}</td>
		...
		<div class="bg-white dark:bg-zinc-900 w-full max-w-md rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
			<div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-900/50">
				<h2 class="text-sm font-black uppercase tracking-widest text-zinc-900 dark:text-zinc-50">
					{isEditing ? 'Ubah Jabatan Fungsional' : 'Tambah Jabatan Fungsional'}
				</h2>
			...
			<div class="p-6 space-y-5">
				...
				<Input
					...
					placeholder="Masukkan nama jabatan fungsional..."
					error={fieldErrors.nmJab}
					required
				/>

				<Input
					label="Jenjang Jabatan ID (UUID)"
					bind:value={formData.jenjangJab_id}
					placeholder="Masukkan UUID jenjang jabatan..."
					error={fieldErrors.jenjangJab_id}
					required
				/>

				<div class="space-y-1.5">
					<label for="kelompok" class="text-[10px] font-black uppercase tracking-widest text-zinc-400">Kelompok</label>
					<select
						id="kelompok"
						bind:value={formData.kelompok}
						class="w-full px-4 py-2.5 bg-zinc-50 dark:bg-zinc-950 border {fieldErrors.kelompok ? 'border-rose-400' : 'border-zinc-200 dark:border-zinc-800'} rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium"
					>
						<option value="Teknis">Teknis</option>
						<option value="Kesehatan">Kesehatan</option>
						<option value="Guru">Guru</option>
					</select>
					{#if fieldErrors.kelompok}
						<p class="text-[10px] font-bold text-rose-500">{fieldErrors.kelompok}</p>
					{/if}
				</div>

				<div class="space-y-1.5">
					<label for="ket" class="text-[10px] font-black uppercase tracking-widest text-zinc-400">Keterangan</label>
					<textarea
						id="ket"
						bind:value={formData.ket}
						class="w-full px-4 py-2.5 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium min-h-[100px]"
