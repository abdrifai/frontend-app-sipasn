...
<div class="space-y-4">
	...
	<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-zinc-100 dark:border-zinc-800/80">
		...
		{#if pegawaiId}
			<button
				...
				class="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 sm:py-1.5 rounded-xl bg-rose-600 hover:bg-rose-700 active:scale-95 text-white text-xs font-semibold shadow-xs transition-all cursor-pointer w-full sm:w-auto"
	...
	{#if riwayat?.length}
		<div class="space-y-3">
			{#each riwayat as rh}
				<div class="p-3.5 sm:p-4 rounded-xl bg-rose-50/40 dark:bg-rose-950/20 border border-rose-200/70 dark:border-rose-900/40 space-y-3 shadow-2xs hover:border-rose-300 dark:hover:border-rose-800/60 transition-all">
					
					<!-- Header Item: Level Badge, Jenis, TMT & Actions -->
					<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b border-rose-100 dark:border-rose-900/40 pb-2.5">
						<div class="flex items-center gap-2 flex-wrap min-w-0">
							<span class="px-2 py-0.5 rounded-lg bg-rose-100 text-rose-800 dark:bg-rose-900/80 dark:text-rose-200 text-[11px] font-bold border border-rose-200 dark:border-rose-800 shrink-0">
								{rh.tingkat}
							</span>
							<span class="font-bold text-xs sm:text-sm text-rose-950 dark:text-rose-100 break-words">
								{rh.jenis}
							</span>
						</div>

						<div class="flex items-center justify-between sm:justify-end gap-2 shrink-0 w-full sm:w-auto">
							<span class="px-2.5 py-1 rounded-lg bg-rose-100/70 dark:bg-rose-900/40 text-[11px] font-semibold text-rose-800 dark:text-rose-300">
					...
					<div class="p-2.5 sm:p-3 rounded-xl bg-white/70 dark:bg-zinc-900/60 border border-rose-100 dark:border-rose-900/30 text-xs text-zinc-700 dark:text-zinc-300 leading-relaxed break-words">
					...
					<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-[11px] text-zinc-500 pt-2 border-t border-rose-100 dark:border-rose-900/30">
						<div class="flex flex-wrap items-center gap-x-4 gap-y-1 min-w-0">
							<span class="break-words">No. SK: <strong class="font-mono text-zinc-700 dark:text-zinc-300 break-all">{rh.sk}</strong> ({rh.tgl_sk})</span>
							<span class="break-words">Dasar Hukum: <strong class="text-zinc-700 dark:text-zinc-300">{rh.no_pp}</strong></span>
							{#if rh.ket && rh.ket !== '-'}
								<span class="break-words">Pejabat/Ket: <strong class="text-zinc-700 dark:text-zinc-300">{rh.ket}</strong></span>
							{/if}
						</div>
						...
						{#if rh.dokumen_sk}
							<a
								...
								target="_blank"
								rel="noreferrer"
								class="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl bg-rose-100 hover:bg-rose-200 dark:bg-rose-900/50 dark:hover:bg-rose-900/80 text-rose-800 dark:text-rose-200 text-xs font-semibold transition-colors w-full sm:w-auto"
	...
	{:else}
		<div class="text-center py-12 px-4 border border-dashed border-emerald-200 dark:border-emerald-900/40 rounded-2xl bg-emerald-50/30 dark:bg-emerald-950/10">
			...
			{#if pegawaiId}
				<button
					type="button"
					onclick={openCreateModal}
					class="mt-3.5 inline-flex items-center justify-center gap-1.5 px-3.5 py-2 sm:py-1.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold shadow-xs transition-all cursor-pointer w-full sm:w-auto"
				>
