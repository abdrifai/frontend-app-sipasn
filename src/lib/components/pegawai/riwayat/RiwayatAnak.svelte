	...
	{#if riwayat?.length}
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
			{#each riwayat as ranak}
				<div class="p-3.5 sm:p-4 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800/80 space-y-3 shadow-2xs hover:border-indigo-200 dark:hover:border-indigo-900/60 transition-all flex flex-col justify-between">
					...
					<div class="space-y-2.5">
						...
						<div class="space-y-0.5">
							<h5 class="font-bold text-sm text-zinc-900 dark:text-zinc-100 break-words">{ranak.nama || '-'}</h5>
							{#if ranak.nik && ranak.nik !== '-'}
								<p class="text-xs text-zinc-500 font-mono">
									NIK: <span class="text-zinc-700 dark:text-zinc-300 font-medium break-all">{ranak.nik}</span>
								</p>
							{/if}
						</div>
						...
						<div class="p-2.5 rounded-lg bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-100 dark:border-zinc-800/60 space-y-1.5 text-xs">
							<div>
								...
								<p class="font-medium text-zinc-800 dark:text-zinc-200 mt-0.5 break-words">{ranak.ttl || '-'}</p>
							</div>
							...
							{#if ranak.nama_ortu && ranak.nama_ortu !== '-'}
								<div class="pt-1 border-t border-zinc-200/40 dark:border-zinc-700/40">
									...
									<p class="font-medium text-indigo-700 dark:text-indigo-300 mt-0.5 break-words">{ranak.nama_ortu}</p>
								</div>
							{/if}
						</div>
							...
							<div class="space-y-1 text-xs text-zinc-600 dark:text-zinc-400 pt-0.5">
								{#if ranak.no_hp && ranak.no_hp !== '-'}
									<p class="flex items-center gap-1.5">
										...
										<span>Kontak: <span class="text-zinc-800 dark:text-zinc-200 font-medium font-mono">{ranak.no_hp}</span></span>
									</p>
								{/if}
								...
								{#if ranak.alamat && ranak.alamat !== '-'}
									<p class="flex items-start gap-1.5 text-[11px]">
										...
										<span class="leading-relaxed break-words">{ranak.alamat}</span>
									</p>
								{/if}
							</div>
						{/if}
					...
					{#if ranak.dokumen_sk || ranak.dokumen_anak}
						<div class="pt-2 border-t border-zinc-100 dark:border-zinc-800/60 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-1.5">
							...
							<a
								...
								target="_blank"
								rel="noreferrer"
								class="inline-flex items-center justify-center gap-1.5 px-2.5 py-1.5 sm:py-1 rounded-lg bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/50 dark:hover:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 font-semibold text-[11px] border border-indigo-200/60 dark:border-indigo-800/60 transition-colors w-full sm:w-auto"
							>
