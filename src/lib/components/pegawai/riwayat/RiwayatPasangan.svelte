	...
	<div class="flex items-center justify-between gap-3 pb-1 border-b border-zinc-100 dark:border-zinc-800/80">
		...
		{#if pegawaiId}
			<button
				...
				class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-pink-600 hover:bg-pink-700 active:scale-95 text-white text-xs font-semibold shadow-xs transition-all cursor-pointer"
	...
	{#if riwayat?.length}
		<div class="grid grid-cols-1 md:grid-cols-2 gap-3.5">
			{#each riwayat as rpas}
				<div class="p-4 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800/80 space-y-3 shadow-2xs hover:border-pink-200 dark:hover:border-pink-900/60 transition-all">
					...
					<div>
						<h5 class="font-bold text-sm text-zinc-900 dark:text-zinc-100">{rpas.nama}</h5>
						<div class="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-zinc-500 font-mono mt-0.5">
							{#if rpas.nik && rpas.nik !== '-'}
								<p>NIK: <span class="text-zinc-700 dark:text-zinc-300 font-medium">{rpas.nik}</span></p>
							{/if}
							{#if rpas.npwp && rpas.npwp !== '-'}
								<p>NPWP: <span class="text-zinc-700 dark:text-zinc-300 font-medium">{rpas.npwp}</span></p>
							{/if}
						</div>
					</div>
					...
					<div class="p-2.5 rounded-lg bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-100 dark:border-zinc-800/60 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
						<div>
							...
							<p class="font-medium text-zinc-800 dark:text-zinc-200 mt-0.5">{rpas.akta_nikah || '-'}</p>
						</div>
						<div>
						...
						{#if rpas.karis_karsu && rpas.karis_karsu !== '-'}
							<div class="sm:col-span-2 pt-1 border-t border-zinc-200/40 dark:border-zinc-700/40">
								...
								<p class="font-mono font-bold text-pink-700 dark:text-pink-300 mt-0.5">{rpas.karis_karsu}</p>
							</div>
						{/if}
						{#if rpas.dokumen_sk || rpas.dokumen_nikah}
							<div class="sm:col-span-2 pt-1.5 border-t border-zinc-200/40 dark:border-zinc-700/40 flex items-center justify-between">
								...
								<a
									...
									target="_blank"
									rel="noreferrer"
									class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-pink-50 hover:bg-pink-100 dark:bg-pink-950/50 dark:hover:bg-pink-900/60 text-pink-700 dark:text-pink-300 font-semibold text-[11px] border border-pink-200/60 dark:border-pink-800/60 transition-colors"
								>
					...
					<div class="space-y-1 text-xs text-zinc-600 dark:text-zinc-400">
						<p class="flex items-center gap-1.5">
							...
							<span>TTL: <strong class="text-zinc-800 dark:text-zinc-200">{rpas.ttl}</strong></span>
						</p>
						...
						{#if rpas.no_hp && rpas.no_hp !== '-'}
							<p class="flex items-center gap-1.5">
								...
								<span>No. HP: <span class="text-zinc-800 dark:text-zinc-200 font-medium">{rpas.no_hp}</span></span>
							</p>
						{/if}
						...
						{#if rpas.alamat && rpas.alamat !== '-'}
							<p class="flex items-start gap-1.5 pt-0.5 text-[11px]">
								...
								<span class="leading-relaxed">{rpas.alamat}</span>
							</p>
						{/if}
						...
						{#if (rpas.akta_cerai && rpas.akta_cerai !== '-') || (rpas.akta_meninggal && rpas.akta_meninggal !== '-')}
							<div class="pt-2 mt-2 border-t border-dashed border-zinc-200 dark:border-zinc-800 space-y-1">
								{#if rpas.akta_cerai && rpas.akta_cerai !== '-'}
									<div class="p-1.5 bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 rounded text-[11px] flex items-center justify-between">
										<span>Akta Cerai: <strong>{rpas.akta_cerai}</strong></span>
										<span>Tgl: {rpas.tgl_cerai || '-'}</span>
									</div>
								{/if}
								{#if rpas.akta_meninggal && rpas.akta_meninggal !== '-'}
									<div class="p-1.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 rounded text-[11px] flex items-center justify-between">
										<span>Surat Kematian: <strong>{rpas.akta_meninggal}</strong></span>
		...
		<div class="text-center py-12 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl bg-zinc-50/50 dark:bg-zinc-900/50">
			...
			{#if pegawaiId}
				<button
					type="button"
					onclick={openCreateModal}
					class="mt-3.5 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-pink-600 hover:bg-pink-700 text-white text-xs font-semibold shadow-xs transition-all cursor-pointer"
				>
					<svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg>
					<span>Tambah Data Pasangan</span>
				</button>
