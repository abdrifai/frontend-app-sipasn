		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-3.5">
			{#each riwayat as ro}
				<div class="p-4 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800/80 space-y-3 shadow-2xs hover:border-indigo-200 dark:hover:border-indigo-900/60 transition-all">
					...
					<!-- Body Card: Nama & NIK -->
					<div>
						<h5 class="font-bold text-sm text-zinc-900 dark:text-zinc-100">{ro.nama}</h5>
						{#if ro.nik && ro.nik !== '-'}
							<p class="text-xs text-zinc-500 font-mono mt-0.5">NIK: <span class="text-zinc-700 dark:text-zinc-300 font-medium">{ro.nik}</span></p>
						{/if}
					</div>
					...
					<div class="space-y-1 text-xs text-zinc-600 dark:text-zinc-400">
						<p class="flex items-center gap-1.5">
							...
							<span>TTL: <strong class="text-zinc-800 dark:text-zinc-200">{ro.ttl}</strong></span>
						</p>
						...
						{#if ro.no_hp && ro.no_hp !== '-'}
							<p class="flex items-center gap-1.5">
								...
								<span>No. HP: <span class="text-zinc-800 dark:text-zinc-200 font-medium">{ro.no_hp}</span></span>
							</p>
						{/if}
						...
						{#if ro.alamat && ro.alamat !== '-'}
							<p class="flex items-start gap-1.5 pt-0.5 text-[11px]">
								...
								<span class="leading-relaxed">{ro.alamat}</span>
							</p>
						{/if}
					</div>
				</div>
