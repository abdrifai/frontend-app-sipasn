<script>
	...
	async function loadLookups() {
		try {
			const res = await api('/ref-jns-hukuman/tkt-lookup');
			tktLookups = res.data;
		} catch (err) {
			console.error('Gagal memuat lookup tingkat hukuman:', err);
		}
	}
	...
	async function loadData() {
		...
		error = null;
		try {
			const res = await api(`/ref-jns-hukuman?search=${search}&page=${page}`);
			data = res.data;
			meta = res.meta;
		} catch (err) {
			error = err.message;
	...
	function openCreate() {
		isEditing = false;
		formData = { id: '', kode: '', tktHukuman_id: '', jnsHukuman: '' };
		fieldErrors = {};
		formError = null;
		showModal = true;
	}
	...
	async function handleSubmit() {
		...
		try {
			const payload = {
				...
				tktHukuman_id: formData.tktHukuman_id,
				jnsHukuman: formData.jnsHukuman
			};

			if (isEditing) {
				await api(`/ref-jns-hukuman/${formData.id}`, {
					method: 'PATCH',
					body: JSON.stringify(payload)
				});
				toast.success('Jenis hukuman berhasil diperbarui');
			} else {
				await api('/ref-jns-hukuman', {
					method: 'POST',
					body: JSON.stringify(payload)
				});
				toast.success('Jenis hukuman berhasil ditambahkan');
			}
			showModal = false;
			loadData();
		} catch (err) {
	...
	async function executeDelete() {
		...
		deleteLoading = true;
		try {
			await api(`/ref-jns-hukuman/${itemToDelete.id}`, { method: 'DELETE' });
			toast.success('Jenis hukuman berhasil dihapus');
...
<div class="space-y-6">
		<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
			<div>
				<h1 class="text-2xl font-bold text-zinc-900 dark:text-zinc-50">Referensi Jenis Hukuman</h1>
				<p class="text-zinc-500 dark:text-zinc-400">Manajemen data kategori hukuman disiplin</p>
			</div>
		...
		<Card>
			<div class="flex gap-2 mb-6">
				<div class="relative flex-1">
					...
					<input
						type="text"
						bind:value={search}
						placeholder="Cari jenis hukuman..."
			...
			{:else if data.length === 0}
				<EmptyState message="Tidak ada data jenis hukuman yang ditemukan." />
			{:else}
						...
						<tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
							{#each data as item}
								<tr class="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
									...
									<td class="px-4 py-4 font-medium text-zinc-900 dark:text-zinc-100">{item.jnsHukuman}</td>
									<td class="px-4 py-4">
										<span class="px-2 py-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 rounded text-xs">
											{item.tkthukuman}
										</span>
									</td>
		...
		<div class="bg-white dark:bg-zinc-900 w-full max-w-md rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
			...
			<div class="p-6 space-y-4">
				...
				<div class="flex flex-col gap-1">
					<label for="tktHukuman_id" class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
						Tingkat Hukuman<span class="text-rose-500 ml-1">*</span>
					</label>
					<select
						id="tktHukuman_id"
						bind:value={formData.tktHukuman_id}
						class="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 rounded text-sm outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all"
						required
					>
						<option value="" disabled>Pilih Tingkat Hukuman</option>
						{#each tktLookups as tkt}
							<option value={tkt.id}>{tkt.tktHukuman}</option>
						{/each}
					</select>
					{#if fieldErrors.tktHukuman_id}
						<p class="text-xs text-rose-500">{fieldErrors.tktHukuman_id}</p>
					{/if}
				</div>

				<Input
					label="Jenis Hukuman"
					bind:value={formData.jnsHukuman}
					placeholder="Masukkan deskripsi jenis hukuman..."
					error={fieldErrors.jnsHukuman}
					required
				/>
			</div>
...
<ConfirmDeleteModal 
	...
	title="Hapus Referensi Jenis Hukuman?"
	message="Anda akan menghapus {itemToDelete?.jnsHukuman}. Tindakan ini tidak dapat dibatalkan melalui sistem."
