<script>
	...
	async function loadData() {
		...
		error = null;
		try {
			const res = await api(`/ref-kedudukan-pns?search=${search}&page=${page}`);
			data = res.data;
			meta = res.meta;
		} catch (err) {
			error = err.message;
	...
	function openEdit(item) {
		...
		formData = { 
			id: item.id.toString(), 
			kedudukanpns: item.kedudukanpns 
		};
		fieldErrors = {};
		formError = null;
		showModal = true;
	...
	async function handleSubmit() {
		...
		try {
			if (isEditing) {
				await api(`/ref-kedudukan-pns/${formData.id}`, {
					method: 'PATCH',
					body: JSON.stringify({
						kedudukanpns: formData.kedudukanpns
					})
				});
				toast.success('Kedudukan PNS berhasil diperbarui');
			} else {
				await api('/ref-kedudukan-pns', {
					method: 'POST',
					body: JSON.stringify({
						kedudukanpns: formData.kedudukanpns
					})
				});
				toast.success('Kedudukan PNS berhasil ditambahkan');
			}
			showModal = false;
			loadData();
		} catch (err) {
	...
	async function executeDelete() {
		...
		deleteLoading = true;
		try {
			await api(`/ref-kedudukan-pns/${itemToDelete.id.toString()}`, { method: 'DELETE' });
			toast.success('Kedudukan PNS berhasil dihapus');
...
<div class="space-y-6">
		<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
			<div>
				<h1 class="text-2xl font-bold text-zinc-900 dark:text-zinc-50">Referensi Kedudukan PNS</h1>
				<p class="text-zinc-500 dark:text-zinc-400">Manajemen data status kedudukan kepegawaian</p>
			</div>
		...
		<Card>
			<div class="flex gap-2 mb-6">
				<div class="relative flex-1">
					...
					<input
						type="text"
						bind:value={search}
						placeholder="Cari kedudukan pns..."
			...
			{:else if data.length === 0}
				<EmptyState message="Tidak ada data kedudukan PNS yang ditemukan." />
			{:else}
					...
					<table class="w-full text-left text-sm">
						<thead class="bg-zinc-50 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800">
							<tr>
								...
								<th class="px-4 py-3">Kedudukan PNS</th>
						...
						<tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
							{#each data as item}
								<tr class="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
									<td class="px-4 py-4 text-zinc-500 dark:text-zinc-400 font-mono text-xs">{item.id}</td>
									<td class="px-4 py-4 font-medium text-zinc-900 dark:text-zinc-100">{item.kedudukanpns}</td>
	...
	<div class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-zinc-950/50 backdrop-blur-sm">
		<div class="bg-white dark:bg-zinc-900 w-full max-w-md rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
			...
			<div class="p-6 space-y-4">
				...
				<Input
					label="Kedudukan PNS"
					bind:value={formData.kedudukanpns}
					placeholder="Masukkan status kedudukan pns..."
					error={fieldErrors.kedudukanpns}
					required
				/>
			</div>
...
<ConfirmDeleteModal 
	...
	title="Hapus Referensi Kedudukan PNS?"
	message="Anda akan menghapus {itemToDelete?.kedudukanpns}. Tindakan ini tidak dapat dibatalkan melalui sistem."
