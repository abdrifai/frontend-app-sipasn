<script>
	...
	async function loadData() {
		...
		error = null;
		try {
			const res = await api(`/ref-pendidikan?search=${search}&page=${page}`);
			data = res.data;
			meta = res.meta;
		} catch (err) {
			error = err.message;
	...
	async function loadTingkat() {
		try {
			const res = await api('/ref-pendidikan/tingkat');
			tingkatOptions = res.data;
		} catch (err) {
			console.error('Failed to load tingkat pendidikan:', err);
		}
	}
	...
	function openCreate() {
		isEditing = false;
		formData = { id: '', tktpend_id: '', pend: '', kode: '' };
		fieldErrors = {};
		formError = null;
		showModal = true;
	}
	...
	async function handleSubmit() {
		...
		try {
			if (isEditing) {
				await api(`/ref-pendidikan/${formData.id}`, {
					method: 'PUT',
					body: JSON.stringify({
						tktpend_id: formData.tktpend_id,
						pend: formData.pend,
						kode: formData.kode
					})
				});
			...
			if (err.statusCode === 422) {
				// Map array errors to fieldErrors object
	...
	async function executeDelete() {
		...
		deleteLoading = true;
		try {
			await api(`/ref-pendidikan/${itemToDelete.id}`, { method: 'DELETE' });
...
<div class="space-y-6">
	<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-zinc-900 dark:text-zinc-50">Referensi Pendidikan</h1>
			<p class="text-zinc-500 dark:text-zinc-400">Manajemen data program studi dan jurusan</p>
		</div>
	...
	<Card>
		<div class="flex gap-2 mb-6">
			<div class="relative flex-1">
				...
				<input
					type="text"
					bind:value={search}
					placeholder="Cari pendidikan atau kode..."
		...
		{:else if data.length === 0}
			<EmptyState message="Tidak ada data pendidikan yang ditemukan." />
		{:else}
				...
				<table class="w-full text-left text-sm">
					<thead class="bg-zinc-50 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800">
						<tr>
							...
							<th class="px-4 py-3">Nama Pendidikan</th>
							...
							<tr class="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
								<td class="px-4 py-4 whitespace-nowrap">
									<span class="px-2 py-1 rounded bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-xs font-bold">
										{item.ref_tktpend?.tktpend || '-'}
									</span>
								</td>
								<td class="px-4 py-4 font-medium text-zinc-900 dark:text-zinc-100">{item.pend}</td>
								<td class="px-4 py-4 text-zinc-500 dark:text-zinc-400">{item.kode || '-'}</td>
	...
	<div class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-zinc-950/50 backdrop-blur-sm">
		<div class="bg-white dark:bg-zinc-900 w-full max-w-md rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
			...
			<div class="p-6 space-y-4">
				...
				<div class="space-y-1">
					<label for="tktpend" class="text-xs font-bold uppercase tracking-wider text-zinc-400">Tingkat Pendidikan</label>
					<select
						id="tktpend"
						bind:value={formData.tktpend_id}
						class="w-full px-4 py-2.5 bg-zinc-50 dark:bg-zinc-950 border {fieldErrors.tktpend_id ? 'border-rose-400' : 'border-zinc-200 dark:border-zinc-800'} rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
					>
						<option value="">Pilih Tingkat Pendidikan</option>
						{#each tingkatOptions as opt}
							<option value={opt.id}>{opt.tktpend}</option>
						{/each}
					</select>
					{#if fieldErrors.tktpend_id}
						<p class="text-[10px] font-bold text-rose-500">{fieldErrors.tktpend_id}</p>
					{/if}
				</div>

				<Input
					label="Nama Pendidikan"
					bind:value={formData.pend}
					placeholder="Masukkan nama jurusan/program studi..."
					error={fieldErrors.pend}
					required
				/>

				<Input
					label="Kode Pendidikan (Opsional)"
...
<ConfirmDeleteModal 
	...
	title="Hapus Referensi Pendidikan?"
	message="Anda akan menghapus {itemToDelete?.pend}. Tindakan ini tidak dapat dibatalkan melalui sistem."
